...
SCANNER_VERSION = "3.0"
SCANNER_VENDOR = "PSADT-Secure"
SCANNER_NAME = "PSADTSecureScanner"
...
class SBOMGenerator:
    ...
    def generate_spdx(self, components: List[dict]) -> str:
        ...
        timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        doc_ns    = f"urn:psadt-secure:{self._package_name}:{self._doc_uuid}"
        ...
        lines = [
            ...
            f"Created: {timestamp}",
            f"CreatorComment: <text>Generated by PSADT-Secure v{SCANNER_VERSION}. "
